...
# Recording Rule reference: https://github.com/Azure/prometheus-collector/blob/main/AddonTerraformTemplate/main.tf

# Create data collection endpoint for AKS metrics
# This endpoint is used to collect and route metrics from the AKS cluster
resource "azurerm_monitor_data_collection_endpoint" "dce" {
  count               = var.enable_managed_prometheus ? 1 : 0
  name                = substr("MSProm-${var.location}-${var.cluster_name}", 0, min(44, length("MSProm-${var.location}-${var.cluster_name}"))) #
...
resource "azurerm_monitor_data_collection_rule" "dcr" {
  ...
  depends_on = [
    ...
    azurerm_monitor_workspace.prometheus[0]
  ]
}
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  count                   = var.enable_managed_prometheus ? 1 : 0
  name                = "NodeRecordingRulesRuleGroup - ${azurerm_kubernetes_cluster.this.name}"
  location            = var.location
  resource_group_name = var.resource_group_name
  ...
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.prometheus[0].id, azurerm_kubernetes_cluster.this.id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules" {
  count                   = var.enable_managed_prometheus ? 1 : 0
  name                = "KubernetesRecordingRulesRuleGroup - ${azurerm_kubernetes_cluster.this.name}"
  location            = var.location
  resource_group_name = var.resource_group_name
  cluster_name        = azurerm_kubernetes_cluster.this.name
...
# Node Recording Rules - Windows (disabled by default)
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_windows" {
  count               = var.enable_managed_prometheus ? 1 : 0
  name                = "NodeRecordingRulesRuleGroup-Win - ${azurerm_kubernetes_cluster.this.name}"
  location            = var.location
  resource_group_name = var.resource_group_name
  cluster_name        = azurerm_kubernetes_cluster.this.name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_kubernetes_recording_rules_windows" {
  count               = var.enable_managed_prometheus ? 1 : 0
  name                = "NodeAndKubernetesRecordingRulesRuleGroup-Win - ${azurerm_kubernetes_cluster.this.name}"
  location            = var.location
  resource_group_name = var.resource_group_name
  cluster_name        = azurerm_kubernetes_cluster.this.name
...
# UX Recording Rules - Linux
# This enable Container Insights
resource "azurerm_monitor_alert_prometheus_rule_group" "ux_recording_rules_linux" {
  count               = var.enable_managed_prometheus ? 1 : 0
  name                = "UXRecordingRulesRuleGroup - ${azurerm_kubernetes_cluster.this.name}"
  location            = var.location
  resource_group_name = var.resource_group_name
  cluster_name        = azurerm_kubernetes_cluster.this.name
